...
- **Recording starts but the second press does nothing.** The toggle is driven by `$XDG_RUNTIME_DIR/voxtyper.state` and `$XDG_RUNTIME_DIR/voxtyper.lock` (or `~/.cache/voxtyper/` on hosts without `XDG_RUNTIME_DIR`). If a previous invocation crashed, the lock or state file can be left behind. Tail `voxtyper-debug.log` in the same directory to see the most recent activity. If every press logs a "Debounce" entry, the shortcut is firing twice — increase `DEBOUNCE_SECONDS` at the top of `voxtyper.sh`.
